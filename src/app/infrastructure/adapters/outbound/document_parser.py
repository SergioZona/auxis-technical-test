...
class PyMuPDFDocumentParser(DocumentParserPort):
    ...
    async def _extract_page_content_node(self, state: ParserState) -> dict[str, Any]:
        ...
        for idx, p_info in enumerate(pages):
            ...
            # Always extract tables as Markdown
            try:
            ...
            if classification == "scanned":
                ...
                # Step 1: OCR first — gives dense text for chunking AND AI context
                try:
                ...
                # Step 2: AI extraction with both image and OCR text as context
                ...
                # Chunk text = OCR (real content) + description appended for context
                ...
                # Step 1: OCR to supplement selectable text
                try:
                    img = Image.open(io.BytesIO(png_bytes))
                    ocr_text = pytesseract.image_to_string(img, lang="spa+eng").strip()
                except Exception as exc:
                ...
                # Chunk text = combined real content + AI description
                desc = ai_data.get("description", "")
                ...
                # Pure text: no image processing needed
    ...
    async def _reconcile_metadata_node(self, state: ParserState) -> dict[str, Any]:
        ...
        # ── Step 2: Merge page-level AI data for any still-missing fields ────
        for p in pages:
        ...
        # ── Step 3: Form 220 positional parser for any remaining numeric gaps ─
        is_form_220 = (
        ...
        # ── Step 4: Regex fallbacks for any remaining empty canonical keys ───
    ...
    def _map_form_220(self, text: str, doc: Document) -> None:
        ...
        if header_idx != -1:
            ...
            employer_name_val = None
            if idx < len(sub) and not sub[idx].isdigit():
                employer_name_val = sub[idx]
                idx += 1
            ...
            doc.form_number = form_num_val
            doc.nit_employer = f"{nit_val}-{dv_val}" if nit_val and dv_val else nit_val
            doc.employer_name = employer_name_val
            doc.employee_document_id = employee_id_val

            if len(name_parts) >= 3:
                lasts = name_parts[:2]
                firsts = name_parts[2:]
                doc.employee_name = " ".join(firsts + lasts)
            elif name_parts:
