...
async def test_query_database_use_case_executes(
    ...
    mock_langchain_rag.query_database.return_value = "Total gross income sum is $123,000,000."
    
    result = await use_case.execute("What is the sum of total gross income?")
    
    mock_langchain_rag.query_database.assert_called_once_with("What is the sum of total gross income?")
